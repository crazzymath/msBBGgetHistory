 ...
 *   In[1]:= link = Install["msBBGgetHistory"]
 *
...
/*		for (int i = 1; i < argc; ++i) {
	...
	std::string sendRefDataRequest(Session &session, const char* startDate, const char* endDate, const char* periodicitySelection, const char* periodicityAdjustment, int useDPDF, int debug)
	{
		...
		request.set("periodicityAdjustment", periodicityAdjustment); /* dying here. */
...
void msBBGgetHistory(const char* ticker, const char* field, const char* startDate, const char* endDate, const char* periodicitySelection = "DAILY", const char* periodicityAdjustment = "Actual", int useDPDF=0, int debug=0)
{
